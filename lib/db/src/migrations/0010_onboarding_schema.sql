...
-- Applied via direct SQL (Drizzle push blocked by schema drift in opportunity_ems_interfacility_profiles)
-- Safe to run multiple times (all CREATE IF NOT EXISTS / ALTER ... IF NOT EXISTS)

-- ─── Enums ────────────────────────────────────────────────────────────────────
DO $$ BEGIN
  CREATE TYPE workspace_add_on_status AS ENUM ('ACTIVE', 'SUSPENDED', 'PENDING_CONFIG');
...
CREATE TABLE IF NOT EXISTS client_onboarding_sessions (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::text,
  status onboarding_session_status NOT NULL DEFAULT 'DRAFT',
...
SELECT v.id, 'ems', 'Emergency Medical Services',
       'EMS providers including ground ambulance, air medical, and interfacility transport', 1
FROM verticals v WHERE v.key = 'healthcare'
ON CONFLICT (vertical_id, key) DO NOTHING;

INSERT INTO sub_verticals (vertical_id, key, label, description, sort_order)
SELECT v.id, 'ambulatory_surgery', 'Ambulatory Surgery Centers',
       'Outpatient surgical facilities and surgery center management', 2
FROM verticals v WHERE v.key = 'healthcare'
ON CONFLICT (vertical_id, key) DO NOTHING;

INSERT INTO sub_verticals (vertical_id, key, label, description, sort_order)
SELECT v.id, 'health_system', 'Health Systems',
       'Multi-facility hospital systems and integrated health networks', 3
FROM verticals v WHERE v.key = 'healthcare'
ON CONFLICT (vertical_id, key) DO NOTHING;

INSERT INTO service_lines (vertical_id, sub_vertical_id, key, label, description, default_pipeline_template_key, sort_order)
SELECT v.id, sv.id, 'bls', 'Basic Life Support (BLS)', 'Ground ambulance BLS transport services',
       'ems_interfacility_transport_v1', 1
FROM verticals v JOIN sub_verticals sv ON sv.key = 'ems' AND sv.vertical_id = v.id
WHERE v.key = 'healthcare'
ON CONFLICT (vertical_id, key) DO NOTHING;

INSERT INTO service_lines (vertical_id, sub_vertical_id, key, label, description, default_pipeline_template_key, sort_order)
SELECT v.id, sv.id, 'als', 'Advanced Life Support (ALS)', 'Ground ambulance ALS transport services',
       'ems_interfacility_transport_v1', 2
FROM verticals v JOIN sub_verticals sv ON sv.key = 'ems' AND sv.vertical_id = v.id
WHERE v.key = 'healthcare'
ON CONFLICT (vertical_id, key) DO NOTHING;

INSERT INTO service_lines (vertical_id, sub_vertical_id, key, label, description, default_pipeline_template_key, sort_order)
SELECT v.id, sv.id, 'cct', 'Critical Care Transport (CCT)', 'Ground or air critical care transport',
       'ems_interfacility_transport_v1', 3
FROM verticals v JOIN sub_verticals sv ON sv.key = 'ems' AND sv.vertical_id = v.id
WHERE v.key = 'healthcare'
ON CONFLICT (vertical_id, key) DO NOTHING;
...
INSERT INTO add_on_types (key, label, description, config_schema) VALUES
  (
    ...
    '{
      "agencyAlignment": {"type":"multiselect","label":"Target Agency Types","options":["Federal","State","Local","DoD","Civilian"],"required":false},
      "contractTypes":  {"type":"multiselect","label":"Contract Types","options":["IDIQ","GWAC","BPA","FFP","T&M","Cost-Plus"],"required":false},
      "ueiRequired":    {"type":"boolean","label":"SAM.gov UEI required","default":true},
      "naicsCodes":     {"type":"text_array","label":"Primary NAICS Codes","required":false},
      "primeOrSub":     {"type":"select","label":"Prime or Subcontractor","options":["prime","sub","both"],"required":false}
    }'::jsonb
  ),
  (
    'hipaa_compliance',
    ...
    '{
      "baaTracking":           {"type":"boolean","label":"Enable BAA tracking","default":true},
      "auditLogRetentionDays": {"type":"integer","label":"Audit log retention (days)","default":365}
    }'::jsonb
  )
