/**
 * NAICS Diagnostics Screen
 *
 * Shows workspace NAICS classification coverage, target alignment,
 * top codes, gaps, and recommendations.
 *
 * Route: /govcon/naics-diagnostics
 */
...
import { useNaicsDiagnostics } from "@/hooks/useGovcon";

function PercentBar({ value, color = COLORS.emerald }: { value: number; color?: string }) {
...
export default function NaicsDiagnosticsScreen() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const { data, isLoading, refetch, isRefetching } = useNaicsDiagnostics();
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>NAICS Diagnostics</Text>
        <View style={[styles.headerIcon, { backgroundColor: COLORS.emerald + "20" }]}>
      ...
      {isLoading ? (
        <View style={styles.loadingWrap}>
          ...
          <Text style={styles.loadingText}>Analyzing NAICS coverage...</Text>
        </View>
      ) : (
        <ScrollView
          ...
          refreshControl={
            <RefreshControl refreshing={isRefetching && !isLoading} onRefresh={refetch} tintColor={COLORS.emerald} />
          }
        >
          {/* Summary stats */}
          ...
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>Classification Coverage</Text>
            <PercentBar value={data?.coveragePercent ?? 0} color={COLORS.emerald} />
          ...
          <View style={styles.section}>
            ...
            <View style={styles.barLabels}>
              <Text style={styles.barLabel}>{data?.alignedOrgs ?? 0} aligned to NAICS targets</Text>
          ...
          {(data?.topNaics?.length ?? 0) > 0 && (
            <View style={styles.section}>
              <Text style={styles.sectionTitle}>Top NAICS Codes</Text>
              {data!.topNaics.map(n => (
                <View key={n.code} style={styles.codeRow}>
                  <View style={[styles.codeChip, { backgroundColor: n.isTargeted ? COLORS.emerald + "20" : COLORS.navySurface }]}>
                    <Text style={[styles.codeText, { color: n.isTargeted ? COLORS.emerald : COLORS.textMuted }]}>{n.code}</Text>
                  </View>
                  <View style={styles.codeInfo}>
                    <Text style={styles.codeName} numberOfLines={1}>{n.title ?? n.code}</Text>
                  ...
                  {n.isTargeted && (
                    <View style={styles.targetedBadge}>
                      ...
                      <Text style={styles.targetedText}>Targeted</Text>
                    </View>
                  )}
                </View>
              ))}
          ...
          {(data?.gaps?.length ?? 0) > 0 && (
            <View style={styles.section}>
              ...
              <Text style={styles.gapDesc}>
                These NAICS codes are in your targeting profile but have no matching classified organizations yet.
              </Text>
...
const styles = StyleSheet.create({
  ...
  targetedText: { fontFamily: "Inter_400Regular", fontSize: 11, color: COLORS.emerald },
