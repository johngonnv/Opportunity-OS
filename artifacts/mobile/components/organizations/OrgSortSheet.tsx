...
export type OrgSortKey = "createdAt" | "updatedAt" | "name" | "city" | "state" | "organizationType";
export type SortOrder = "asc" | "desc";

const SORT_OPTIONS: { key: OrgSortKey; label: string; ascLabel: string; descLabel: string }[] = [
  ...
  { key: "name",             label: "Name",         descLabel: "Z → A",             ascLabel: "A → Z" },
  { key: "city",             label: "City",         descLabel: "Z → A",             ascLabel: "A → Z" },
  { key: "state",            label: "State",        descLabel: "Z → A",             ascLabel: "A → Z" },
  { key: "organizationType", label: "Type",         descLabel: "Z → A",             ascLabel: "A → Z" },
];
...
type Props = {
  ...
  sortBy: OrgSortKey;
  sortOrder: SortOrder;
  onChange: (sortBy: OrgSortKey, sortOrder: SortOrder) => void;
};

export function OrgSortSheet({ visible, onClose, sortBy, sortOrder, onChange }: Props) {
  const insets = useSafeAreaInsets();

  const handleSelect = (key: OrgSortKey, order: SortOrder) => {
    onChange(key, order);
    onClose();
  };
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      ...
      <View style={[styles.sheet, { paddingBottom: insets.bottom + 16 }]}>
        ...
        <Text style={styles.title}>Sort Organizations</Text>
...
const styles = StyleSheet.create({
  ...
  sheet: {
    ...
    paddingHorizontal: 16, paddingTop: 12, maxHeight: "75%",
  },
