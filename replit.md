...
- **Contact Identity v1 Foundation (Task #55):** Adds soft-delete (`deleted_at`) to contacts/orgs/master_contacts/master_orgs, normalized E.164 phone columns, identity fingerprint on master contacts (sha256 of work-email + work-phone + master-org), `contact_channels` (work/personal/mobile/home channels with the WORK label flowing up to master), `master_contact_employment_log` (append-only org/title/department history written on every promotion or merge), and `master_merge_queue` (admin review queue for non-auto-mergeable matches). Partial unique indexes enforce email-based identity per workspace and per master-org; phone uniqueness is intentionally NOT enforced because legitimate shared-office phones exist (e.g., front-desk lines). Write-time DUPLICATE 409s on contact + master-org/master-contact create paths. Admin restore endpoints under `/admin/contact-identity/*`. Audit log entries with before/after JSON on promote, merge, soft-delete, restore, and merge-queue resolve.
